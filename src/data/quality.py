...
def check_class_balance(df: pd.DataFrame) -> dict:
    ...
    logger.info(
        ...
        win_rate if not np.isnan(win_rate) else -1, n_wins, n_total,
    )
    return result
...
def run_feature_checks(
    ...
    for feat in features:
        ...
        year_coverage = (
            int(df.loc[df[feat].notna(), YEAR_COL].nunique())
            if YEAR_COL in df.columns else -1
        )
        ...
        records.append({
            ...
            "p25": round(float(col.quantile(0.25)), 4) if len(col) > 0 and is_numeric else np.nan,
            "median": round(float(col.median()), 4) if len(col) > 0 and is_numeric else np.nan,
            "p75": round(float(col.quantile(0.75)), 4) if len(col) > 0 and is_numeric else np.nan,
...
def run_quality_checks(
    ...
    if features is None:
        features = sorted(
            ...
            if c.startswith("delta_") or c == "top3_gp_pct_delta"
        )
        if not features:
    ...
    for slice_name, slice_df in slices.items():
        if slice_df.empty:
            logger.info(
                "Skipping window '%s' — no rows in dataset for this period.", slice_name
            )
            continue
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        ...
        if dup_check.get("n_duplicates", 0) == 0:
            ...
            f.write(f"  FAIL  {dup_check['n_duplicates']} duplicate IDs found: "
        ...
        for window, group in report.groupby("window"):
            ...
            if window == "full":
                ...
                for _, row in group.sort_values("feature").iterrows():
                    ...
                    mean_ = f"{row.get('mean', float('nan')):.2f}" if pd.notna(row.get("mean")) else "  —"
                    std_ = f"{row.get('std', float('nan')):.2f}" if pd.notna(row.get("std")) else "  —"
                    min_ = f"{row.get('min', float('nan')):.2f}" if pd.notna(row.get("min")) else "  —"
                    med_ = f"{row.get('median', float('nan')):.2f}" if pd.notna(row.get("median")) else "  —"
                    max_ = f"{row.get('max', float('nan')):.2f}" if pd.notna(row.get("max")) else "  —"
                    out_ = f"{row.get('outlier_rate', float('nan')):.1%}" if pd.notna(row.get("outlier_rate")) else "  —"
                    f.write(
